# Предобработка публицистических текстов
...
STOPWORDS = {
    ...
    'со','без','во','ты','тебя','тебе','им','ему','нас','нам','они','них',
    'ними','этот','эта','эти','тот','та','те','свой','своя','свое','свои',
...
if __name__ == '__main__':
    df_raw = pd.read_csv('/Users/lidia.donts/Documents/учеба/python_projects/text-styles/cmd/datasets/journalistic_2000.csv')

    df = pd.DataFrame()
    df['text']     = df_raw['text'].astype(str)
    df['label']    = 2
    df['style']    = 'journalistic'
    df['style_ru'] = 'Публицистический'

    # Применяем pipeline
    df['text_ready']  = df['text'].apply(preprocess)
    df['token_count'] = df['text_ready'].str.split().str.len()

    # Фильтруем слишком короткие тексты после очистки
    before = len(df)
    ...
    df[['text_ready', 'label', 'style', 'style_ru']].to_csv(
        'journalistic_preprocessed.csv',
        index=False, encoding='utf-8-sig'
    )
    print('Сохранено: journalistic_preprocessed.csv')
