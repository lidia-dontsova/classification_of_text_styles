# Предобработка официально-деловых текстов
...
def remove_artifacts(text: str) -> str:
    ...
    text = re.sub(r'\bN\s*\d+[\-\w]*', ' ', text)
    text = re.sub(r'\bст\.\s*\d+', ' ', text)
    return text
...
if __name__ == '__main__':
    df = pd.read_csv('/Users/lidia.donts/Documents/учеба/python_projects/text-styles/cmd/datasets/formal_2000.csv')
    ...
    df[['text_ready', 'label', 'style', 'style_ru']].to_csv(
        'formal_preprocessed.csv',
        index=False, encoding='utf-8-sig'
    )
    print('Сохранено: formal_preprocessed.csv')
